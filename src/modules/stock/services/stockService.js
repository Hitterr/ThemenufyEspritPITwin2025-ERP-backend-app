...
class StockService {
  ...
  async getSuppliersForStock(stockId) {
    ...
    const calculateCompositeScore = (pricePerUnit, qualityScore, leadTimeDays) => {
      const minPrice = 1;
      const maxPrice = 10;
      const minLeadTime = 1;
      const maxLeadTime = 7;
      ...
      const normalizedLeadTime = 1 - normalize(leadTimeDays, minLeadTime, maxLeadTime);
      const score = (
        0.4 * normalizedPrice +
    ...
    const comparisonData = await Promise.all(
      suppliers.map(async (supplier) => {
        ...
        try {
          qualityScore = await SupplierService.calculateDynamicQualityScore(supplier._id);
