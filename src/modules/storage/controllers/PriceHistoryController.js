...
exports.createPriceHistory = async (req, res) => {
  try {
    const { stockId, restaurantId, price, invoiceId, supplierId } = req.body;
...
exports.getPriceHistories = async (req, res) => {
  try {
    const { restaurantId, stockId, supplierId, invoiceId } = req.query;

    // Au moins un filtre doit être présent
    if (!restaurantId && !stockId && !supplierId && !invoiceId) {
      return res.status(400).json({
