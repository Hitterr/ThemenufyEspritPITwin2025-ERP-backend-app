...
exports.getConsumption = async (req, res) => {
  try {
    const { restaurantId, stockId, ordreId } = req.query;
    // Vérification des paramètres de la requête
    if (!restaurantId && !stockId && !ordreId) {
      return res.status(400).json({
        message:
          "Au moins un des paramètres (restaurantId ou stockId ou ordreId) doit être fourni.",
      });
    }
