...
exports.createConsumption = async (req, res) => {
  try {
    ...
    if (!restaurantId || !ingredientId || !ordreId||!qty) {
      return res.status(400).json({ message: "Tous les paramètres (restaurantId, ingredientId,ordreId, qty) sont requis." });
    }

    // Appel à la fonction de création
    const result = await historyService.createConsumption(ingredientId, restaurantId,ordreId, qty);
...
exports.getConsumption = async (req, res) => {
  try {
    ...
    if (!restaurantId && !ingredientId && !ordreId) {
      return res.status(400).json({ message: "Au moins un des paramètres (restaurantId ou ingredientId ou ordreId) doit être fourni." });
    }

    // Appel à la fonction de récupération des consommations
    const results = await historyService.getConsumptions(restaurantId, ingredientId, ordreId);
